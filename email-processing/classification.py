...
TRADE = re.compile(r"\b(buy|sell|long|short|pay|receive|entry|exit|target|stop|tp|sl|rv|curve|spread)\b", re.I)
TENOR = re.compile(r"\b(\d{1,2}y|\d{1,2}m|\d{1,2}w|\d{1,2}d|[23]m|6m|2s10s|5s30s)\b", re.I)
CLIENT = re.compile(r"\b(client|rfq|enquiry|request|mandate|escalate)\b", re.I)
INTERNAL = re.compile(r"\b(internal|rota|shift|coverage|action required|approval|review|handover)\b", re.I)
MARKET = re.compile(r"\b(market update|am bullet|pm bullet|opening bell|futures|headline|color|wrap|snapshot)\b", re.I)
MATERIALS = re.compile(r"\b(attached|attachment|slides|deck|pptx|pdf|chartbook|appendix)\b", re.I)
RESEARCH = re.compile(r"\b(research|note|preview|outlook|publication|strategy|house view)\b", re.I)
...
class RuleBasedClassifier:
    ...
    def classify(self, subject: str, body: str, from_addr: Optional[str] = None) -> ClassificationResult:
        subject = subject or ""
        body = body or ""
        domain = (from_addr or "").split("@")[-1].lower() if from_addr and "@" in from_addr else ""
        ...
        if domain.endswith(".internal") or INTERNAL.search(subject) or INTERNAL.search(body):
        ...
        if MATERIALS.search(subject) or MATERIALS.search(body):
            return ClassificationResult("Materials", 0.65, "rules", "materials_reference")
        if RESEARCH.search(subject) or RESEARCH.search(body):
            return ClassificationResult("Research", 0.6, "rules", "research_terms")
        return ClassificationResult("Research", 0.4, "rules", "fallback_research")
...
class LLMEmailClassifier:
    ...
    def __init__(self, model: Optional[str] = None, endpoint: Optional[str] = None, temperature: float = 0.0):
...
class HybridClassifier:
    ...
    def __init__(self, enable_llm: Optional[bool] = None, threshold: float = 0.7):
        ...
        self.enable_llm = enable_llm if enable_llm is not None else env_flag in {"1", "true", "yes"}
