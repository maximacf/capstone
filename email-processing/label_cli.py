...
def fetch_next_unlabeled(con: sqlite3.Connection) -> Optional[tuple]:
    cur = con.execute(
        """
...
def apply_label(con: sqlite3.Connection, msg_id: str, label: str) -> None:
    con.execute(
        "UPDATE message SET manual_category = ?, updated_ts = datetime('now') WHERE id = ?",
        (label, msg_id),
    )
    con.commit()
...
def main():
    ...
    parser.add_argument("--db", default="data/mail.db", help="Path to SQLite database.")
